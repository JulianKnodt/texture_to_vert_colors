...
macro_rules! define_det {
  ($p: vis $name: ident, $name2: ident,
    ...
    #[inline]
    $p fn $name2 <const COL: usize, $(const $x: usize, const $y: usize,)+>(&self, col: [F; 4]) -> F {
      let mut acc = 0.;
      $(
...
impl SymMatrix4 {
    ...
    pub fn col(&self, j: usize) -> [F; 4] {
        from_fn(|i| self.v(j, i))
    }

    #[inline]
    pub fn qr(&self) -> ([[F; 4]; 4], [[F; 4]; 4]) {
        // [col][row]
        let mut v: [[F; 4]; 4] = from_fn(|i| self.col(i));
        let mut q = [[0.; 4]; 4];
        let mut r = [[0.; 4]; 4];
    ...
    pub fn eigen_3x3(&self) -> ([F; 3], [[F; 3]; 3]) {
        ...
        let (e, v) = match f.abs() < ZERO_EPS {
            ...
            false | true => {
                let mat3 = self.to_3x3();
                let vs = super::svd::eigen_jacobi(mat3);
                let es = vs.map(|v| dot(v, mat3.vec_mul(v)));
                (es, vs)
            }
        };
    ...
    define_det!(det3x3, det3x3_with_col, X0, [
      +=X0, Y0 -> [X1, Y1, X2, Y2],
      -=X1, Y1 -> [X1, Y0, X2, Y2],
      +=X2, Y2 -> [X1, Y0, X2, Y1]
    ], det2x2, det2x2_with_col);

    define_det!(det4x4, det4x4_with_col, X0, [
    ...
    */
    ...
    pub fn vec_mul(&self, v: [F; 4]) -> [F; 4] {
        from_fn(|i| (0..4).map(|j| self.v(i, j) * v[i]).sum())
    }

    pub fn vec_mul3(&self, v: [F; 3]) -> [F; 3] {
        from_fn(|i| (0..3).map(|j| self.v(i, j) * v[i]).sum())
    }
    ...
    pub fn flat<const N: usize>(&self) -> [[F; N]; N] {
        from_fn(|i| from_fn(|j| self.v(i, j)))
    }
...
impl SymMatrix3 {
    ...
    // TODO accuracy of this is questionable, may need to fix it.
    pub fn power_eigen(&self) -> ([F; 3], [[F; 3]; 3]) {
        let mut q0 = [1., 0., 0.];
        let mut q1 = [0., 1., 0.];
        //let mut q2 = [1., 0., 0.];
        use super::{cross, dot, kmul, length, normalize, sub};
        // FIXME somehow increasing this value doesn't always increase the robustness of
        // computation.
        for _ in 0..64 {
            let mut any = false;
            ...
            // this step makes convergence faster?
            //let eig0 = dot(self.vec_mul(q0), q0);
            //let n_q1 = (*self - Self::diag([eig0; 3])).vec_mul(q1);
        ...
        // Need to do this to ensure q1 is orthogonal
        q1 = normalize(sub(q1, kmul(dot(q1, q0), q0)));
    ...
    pub fn eigen_sorted(&self) -> ([F; 3], [[F; 3]; 3]) {
        let (es, vs) = self.eigen();
        let mut ord = [0, 1, 2];
        ord.sort_unstable_by(|&a, &b| F::total_cmp(&es[a], &es[b]));
        (ord.map(|i| es[i]), ord.map(|i| vs[i]))
    }
    ...
    pub fn eigen(&self) -> ([F; 3], [[F; 3]; 3]) {
        ...
        let es = vs.map(|v| dot(v, self.vec_mul(v)));
        (es, vs)
    }

    #[inline]
    pub fn trace(&self) -> F {
        self.v_const::<0, 0>() + self.v_const::<1, 1>() + self.v_const::<2, 2>()
    }

    #[inline]
    /// Returns the rotation from this 3x3 matrix to another
    // TODO maybe should include scaling
    pub fn angle_between(&self, o: &Self) -> F {
        let (_es, [v0, v1, _v2]) = self.eigen();
        let (_oes, [ov0, ov1, _ov2]) = o.eigen();

        let q = super::quat_from_basis(v0, v1, ov0, ov1);
        q[3].abs().acos() * 2. + length(sub(_es, _oes))
    }

    #[inline]
    /// Returns rotationally symmetric cross-field symmetry
    pub fn edge_min_similarity(&self, _pos: [F; 3], edges: impl Iterator<Item = [F; 3]>) -> F {
        let (_es, vs) = self.eigen();
        let [v0, v1, v2] = vs.map(normalize);
        edges
            .map(normalize)
            .map(|e| 1. - dot(v0, e).abs().max(dot(v1, e).abs()).max(dot(v2, e).abs()))
            .sum::<F>()
    }
}
